  ...
  <div class="space-y-8">
    ...
    <div class="card backdrop-blur">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-neutral-900 dark:text-white">Plans Management</h1>
        <span class="text-sm text-neutral-600 dark:text-neutral-400">Total: {{ adminPlansStore.totalPlans }} ({{ adminStore.totalPlans }})</span>
      </div>
      <div class="flex justify-end">
        <button @click="showModal = true; isEdit = false; form = { id: 0, name: '', price: 0, discounted_price: undefined, features: [], is_active: true }; featuresText = '';" class="btn-3d">
          Create New Plan
        </button>
      </div>
    </div>
    ...
    <!-- Plans Table Card -->
    <div class="card backdrop-blur">
      <h2 class="text-2xl font-semibold text-neutral-900 dark:text-white mb-4">All Plans</h2>

      <div v-if="adminPlansStore.loading" class="flex justify-center items-center h-64">
        <p class="text-base text-neutral-600 dark:text-neutral-400">Loading plans...</p>
      </div>
        ...
        <table class="w-full table-auto">
          <thead>
            <tr class="bg-neutral-50 dark:bg-neutral-800">
              ...
              <th class="px-4 py-3 text-left text-sm font-semibold text-neutral-900 dark:text-white">Price</th>
              <th class="px-4 py-3 text-left text-sm font-semibold text-neutral-900 dark:text-white">Discounted Price</th>
              <th class="px-4 py-3 text-left text-sm font-semibold text-neutral-900 dark:text-white">Features</th>
          ...
          <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
            ...
            >
              <td class="px-4 py-4 whitespace-nowrap font-medium text-neutral-900 dark:text-white">
                {{ plan.name }}
              </td>
              ...
              <td class="px-4 py-4 text-sm text-neutral-500 dark:text-neutral-400 max-w-xs">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click="showModal = false">
      <div class="card backdrop-blur max-w-md w-full max-h-[90vh] overflow-y-auto" @click.stop>
        <h3 class="text-xl font-semibold text-neutral-900 dark:text-white mb-4">{{ isEdit ? 'Edit Plan' : 'Create Plan' }}</h3>
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <!-- Discounted Price -->
          <div>
            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Discounted Price ($)</label>
            <input v-model.number="form.discounted_price" type="number" min="0" step="0.01" class="input"/>
          </div>
          ...
          <div class="flex items-center">
            <input id="plan_active" v-model="form.is_active" type="checkbox" class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-neutral-300 dark:border-neutral-600 rounded"/>
            <label for="plan_active" class="ml-2 block text-sm text-neutral-900 dark:text-white">Is Active</label>
          </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="card backdrop-blur max-w-sm w-full">
        ...
        <p class="text-neutral-600 dark:text-neutral-400 mb-6 text-sm">Are you sure you want to permanently delete this plan? This action cannot be undone.</p>
